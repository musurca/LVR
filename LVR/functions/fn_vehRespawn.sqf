...
Author: Iceman77
as modified by OppressorMan (5/5/2019)
to mirror behavior in BIS Vehicle Respawn
...
private ["_veh","_abandonDelay","_destroyedDelay","_vehInit","_abandonDist","_abandonTime",
         "_deadTime","_vehName","_vehDir","_vehPos","_vehtype","_abandoned","_dead"];
...
while { true } Do {
	...
	if ( ( alive _veh ) && { ( canMove _veh ) && { { ( alive _x ) } count ( crew _veh ) == 0 } } &&
	     ( _veh distance _vehPos > 50 ) && ({ _veh distance _x < _abandonDist } count allPlayers == 0) ) then {
		_abandonTime = _abandonTime + 5;
		
		// IF THE VEHICLE IS ABANDONED THEN DELETE IT AND CREATE A NEW ONE AT THE STARTING POSITION
		if ( _abandonTime > _abandonDelay ) then {
			deleteVehicle _veh;
			sleep 1;
			...
			_abandonTime=0;	// reset abandon time
		};
	} else {
		_abandonTime=0; // reset abandon time
	};
